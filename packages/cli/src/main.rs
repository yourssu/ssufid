...
use ssufid_inso::SsufidInsoPlugin;
use ssufid_itsites::{
    cse::{
...
impl SsufidPluginRegistry {
    async fn save_run(
        ...
        match self {
            ...
            SsufidPluginRegistry::Inso(plugin) => {
                save_run(core, out_dir, plugin, posts_limit, retry_count).await
            }
        }
    }
...
fn construct_tasks(
    ...
    let tasks = [
        ...
        (
            SsufidInsoPlugin::IDENTIFIER,
            SsufidPluginRegistry::Inso(SsufidInsoPlugin::default()),
        ),
    ];
