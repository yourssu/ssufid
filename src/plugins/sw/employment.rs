...
pub struct SwEmploymentPlugin {
    crawler: ITCrawler<Self>,
}

impl SsufidPlugin for SwEmploymentPlugin {
    const IDENTIFIER: &'static str = "sw.ssu.ac.kr/employment";
    const TITLE: &'static str = "숭실대학교 컴퓨터학부 취업정보";
    const DESCRIPTION: &'static str = "숭실대학교 컴퓨터학부 홈페이지의 취업정보를 제공합니다.";
    const BASE_URL: &'static str = "https://sw.ssu.ac.kr/bbs/board.php?bo_table=employment";
...
impl Default for SwEmploymentPlugin {
    fn default() -> Self {
        Self::new()
    }
}

impl SwEmploymentPlugin {
    fn new() -> Self {
        Self {
...
mod tests {
    ...
    async fn test_crawl() {
        let posts_limit = 100;
        let plugin = SwEmploymentPlugin::new();
