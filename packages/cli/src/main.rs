...
use ssufid_infocom::SsuInfocomPlugin;
use ssufid_itsites::{
    cse::{
...
impl SsufidPluginRegistry {
    async fn save_run(
        ...
        match self {
            ...
            SsufidPluginRegistry::Infocom(plugin) => {
                save_run(core, out_dir, plugin, posts_limit, retry_count).await
            }
        }
    }
...
fn construct_tasks(
    ...
    let tasks = [
        ...
        (
            SsuInfocomPlugin::IDENTIFIER,
            SsufidPluginRegistry::Infocom(SsuInfocomPlugin::default()),
        ),
    ];
